...
public class ProfileSetup extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (editing) {
            editingRowID = extras.getInt(DBAdapter.KEY_ROWID);
            name = extras.getString(DBAdapter.KEY_PROF_NAME);
            seed = extras.getString(DBAdapter.KEY_SEED);
            otpType = extras.getInt(DBAdapter.KEY_OTP_TYPE);
            digits = extras.getInt(DBAdapter.KEY_DIGITS);
            zone = extras.getString(DBAdapter.KEY_TIME_ZONE);
            if (otpType == Home.OTP_TYPE_MOTP) {
                ...
                ArrayAdapter<CharSequence> seedTypesAdapter = ArrayAdapter
                        .createFromResource(getApplicationContext(),
                                R.array.SeedTypes,
                                android.R.layout.simple_spinner_item);
                seedTypesAdapter
                ...
                ArrayAdapter<CharSequence> timeIntervalAdapter = ArrayAdapter
                        .createFromResource(getApplicationContext(),
                                R.array.TimeIntervals,
                                android.R.layout.simple_spinner_item);
                timeIntervalAdapter
                        .setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
            ...
            if (type == Home.OTP_TYPE_MOTP) {
                ...
                ArrayAdapter<CharSequence> seedTypeAdapter = ArrayAdapter
                        .createFromResource(getApplicationContext(),
                                R.array.SeedTypes,
                                android.R.layout.simple_spinner_item);
                seedTypeAdapter
    ...
    private OnClickListener motpSaveButtonListener = new OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            if (checkIfInDatabase(name) == true) {
                builder.setTitle(getString(R.string.error_title));
                builder.setMessage(getString(R.string.error));
                builder.setPositiveButton(getString(R.string.ok), null);
                builder.show();
            ...
            if (editing == false) {
                insertIntoDatabaseAndFinish(name, seed, Home.OTP_TYPE_MOTP, 0,
                        0, time_zone, 30);
            }
            else {
                updateIntoDatabaseAndFinish(name, seed, Home.OTP_TYPE_MOTP, 0,
                        time_zone, 30);
            }
        }
    };

    private OnClickListener hotpSaveButtonListener = new OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            String name = profName.getText().toString();
            String seed = profSeed.getText().toString();
            boolean hexadecimalSeed = seedTypeSpinner.getSelectedItemPosition() == 0 ? true
                    : false;
            int digits = Integer.parseInt(profDigit.getText().toString());
            if (hexadecimalSeed == true) {
                try {
                    if (seed.length() % 2 != 0) {
            ...
            if (editing == false) {
                insertIntoDatabaseAndFinish(name, seed, Home.OTP_TYPE_HOTP, 0,
                        digits, "", 30);
            }
            else {
                updateIntoDatabaseAndFinish(name, seed, Home.OTP_TYPE_HOTP,
                        digits, "", 30);
            }
        }
    };

    private OnClickListener totpSaveButtonListener = new OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            boolean hexadecimalSeed = seedTypeSpinner.getSelectedItemPosition() == 0 ? true
                    : false;
            int timeInterval = timeIntervalSpinner.getSelectedItemPosition() == 0 ? 30
                    : 60;
            if (digits <= 0 || digits >= 10) {
                builder.setTitle(R.string.error_title);
                builder.setMessage(R.string.digit_error);
            ...
            if (editing == false) {
                insertIntoDatabaseAndFinish(name, seed, Home.OTP_TYPE_TOTP, 0,
                        digits, "", timeInterval);
            }
            else {
                updateIntoDatabaseAndFinish(name, seed, Home.OTP_TYPE_TOTP,
                        digits, "", timeInterval);
            }
        }
    ...
    private void insertIntoDatabaseAndFinish(String name, String seed,
            ...
            int time_interval) {
        db.open();
        int rowId = (int) db.insertProfile(name, seed, otpType, digits,
                time_zone, time_interval);
        db.close();
        ...
        ed.putInt(DBAdapter.KEY_ROWID, rowId);
        ed.putInt(DBAdapter.KEY_TIME_INTERVAL, time_interval);
        ed.commit();
        finish();
    }

    private void updateIntoDatabaseAndFinish(String name, String seed,
            int otpType, int digits, String time_zone, int time_interval) {
        db.open();
    ...
    public static String positionToZone(int zone) {
        switch (zone) {
            case 0:
                return "GMT-12:00";
            case 1:
    ...
    public static int zoneToPosition(String zone) {
